...
async function handleOverride(thread: vscode.CommentThread | undefined): Promise<void> {
  ...
  const overrideComment: vscode.Comment = {
    body: new vscode.MarkdownString(
      '_Region marked as overridden without verification. Use this only when the AI code is wrong and you plan to delete or rewrite it._'
    ),
    mode: vscode.CommentMode.Preview,
    author: { name: 'VibeCheck' },
  ...
  thread.canReply = false;
  thread.collapsibleState = vscode.CommentThreadCollapsibleState.Collapsed;
  regionTracker.markStatus([meta.regionId], 'overridden');
 ...
 */
async function handleSkip(thread: vscode.CommentThread | undefined): Promise<void> {
  thread = resolveThread(thread);
  if (!thread) {
    vscode.window.showWarningMessage(
