...
function handlePanelMessage(message) {
    ...
    if (msg.type === 'answer') {
        vscode.window.showInformationMessage(`VibeCheck: answer selected for ${msg.changedFunction ?? 'function'}.`);
        return;
    }
    if (msg.type === 'skip') {
    ...
    if (msg.type === 'explain') {
        vscode.window.showInformationMessage(`VibeCheck: explain requested for ${msg.changedFunction ?? 'function'}.`);
        return;
    }
