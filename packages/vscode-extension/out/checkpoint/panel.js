...
let resolvedCheckpointIds = new Set();
const BACKEND_URL = process.env.BACKEND_URL || 'http://127.0.0.1:8000';
const KNOWLEDGE_LOG_FILE = process.env.VIBECHECK_KNOWLEDGE_LOG_FILE || '.vibecheck-knowledge.jsonl';
function openCheckpointPanel(context, sessionId, questions, trigger) {
    currentSessionId = sessionId;
    currentQuestions = normalizeQuestions(questions);
    resolvedCheckpointIds = new Set();
    if (currentPanel) {
...
async function handlePanelMessage(message, context) {
    const msg = (message ?? {});
    if (msg.type !== 'pass' &&
        msg.type !== 'fail' &&
        msg.type !== 'answer' &&
    ...
    if (msg.type === 'answer') {
        ...
        try {
            ...
            vscode.window.showInformationMessage(`VibeCheck: scored ${Math.round(score.overall * 100)}% (${score.passed ? 'pass' : 'needs work'}) for ${msg.changedFunction ?? 'function'}.`);
    ...
    if (msg.type === 'skip') {
        vscode.window.showWarningMessage(`VibeCheck: skipped ${msg.changedFunction ?? 'function'}.`);
    ...
    if (msg.type === 'explain') {
        ...
        if (!followUpAnswer || !followUpAnswer.trim()) {
            vscode.window.showWarningMessage(`VibeCheck: follow-up unanswered for ${msg.changedFunction ?? 'function'}.`);
            return;
        }
        try {
            const score = await verifyAnswer({ ...question, question: followUp, whyThisMatters: explanation }, followUpAnswer.trim());
            logKnowledge(context, {
                sessionId: currentSessionId ?? 'unknown',
                checkpointId: question.checkpointId ?? checkpointId,
                changedFunction: question.changedFunction ?? 'unknown',
            ...
            vscode.window.showInformationMessage(`VibeCheck: follow-up captured for ${msg.changedFunction ?? 'function'} (${Math.round(score.overall * 100)}%).`);
            markCheckpointResolved(question.checkpointId ?? checkpointId);
            currentPanel?.webview.postMessage({
...
function estimateOverallImpact(items) {
...
function markCheckpointResolved(checkpointId) {
    if (!checkpointId) {
        return;
    }
...
function maybeAutoPass() {
    ...
    vscode.window.showInformationMessage('VibeCheck: all questions handled. PASS signal created automatically.');
}
...
function buildSimplerFollowUp(question) {
    const fn = question.changedFunction ?? 'this function';
    return `In one sentence, what is the most important behavior change in ${fn}?`;
}
function verifyAnswer(question, transcript) {
    const diffExcerpt = resolveDiffExcerpt(question);
    return postJson(`${BACKEND_URL}/gate/verify`, {
...
function resolveDiffExcerpt(question) {
        ...
        const req = client.request({
            ...
            res.on('end', () => {
                if ((res.statusCode ?? 500) < 200 || (res.statusCode ?? 500) >= 300) {
                    reject(new Error(`HTTP ${res.statusCode ?? 500}: ${responseBody.slice(0, 400)}`));
                    return;
                }
                try {
...
function logKnowledge(context, record) {
    const workspaceRoot = vscode.workspace.workspaceFolders?.[0]?.uri.fsPath;
    if (!workspaceRoot) {
        return;
    }
